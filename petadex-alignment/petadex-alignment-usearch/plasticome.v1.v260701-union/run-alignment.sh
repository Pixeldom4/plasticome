#!/usr/bin/env bash
# All-vs-all alignment for the plasticome.v1.v260701-union all-centroids set, using
...
# Same shape as the singleton-cleaned-union.v1.v2 run; results land in ./results/.
set -euo pipefail
cd "$(dirname "$0")"                                   # -> plasticome.v1.v260701-union/
...
FASTA=plasticome.v1.v260701-union.all-centroids.fasta
OUT=results
mkdir -p "$OUT"
...
usearch() {  # $1 = fasta, $2 = out pairs  (both repo-relative, under the /d mount)
  docker run --rm --platform linux/amd64 -v "$(cd "$REPO" && pwd)":/d -w /d debian:bookworm-slim \
    /d/analysis/bin/usearch11 -allpairs_local "/d/plasticome.v1.v260701-union/$1" \
    -userout "/d/plasticome.v1.v260701-union/$2" \
