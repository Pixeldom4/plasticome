#!/usr/bin/env bash
# All-vs-all alignment for the plasticome-union v1.1 all-centroids set, using the
# frozen PETadex component pipeline (analysis/, v4-v8 paper-faithful method):
...
# Same shape as the v11 centroids run; results land in ./results/.
set -euo pipefail
cd "$(dirname "$0")"                                   # -> plasticome.union.v1.v2/
...
FASTA=plasticome-union-v1.1.all-centroids.fasta
OUT=results
mkdir -p "$OUT"
...
usearch() {  # $1 = fasta, $2 = out pairs  (both repo-relative, under the /d mount)
  docker run --rm --platform linux/amd64 -v "$(cd "$REPO" && pwd)":/d -w /d debian:bookworm-slim \
    /d/analysis/bin/usearch11 -allpairs_local "/d/plasticome.union.v1.v2/$1" \
    -userout "/d/plasticome.union.v1.v2/$2" \
