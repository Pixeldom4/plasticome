#!/usr/bin/env python3
"""Steps 2-3 (v4) - flat similarity graph + connected-component partition.

Edge criterion (post-processing only, never a search parameter):
    >=30% amino-acid identity AND e-value < 1e-5
Partition: undirected single-linkage connected components over ALL 476 nodes.

This is a FLAT de-novo run (decisions 1-4):
  * No v1 anchor, no delta, no join/merge/novel typing.
  * E-values are thresholded AS REPORTED by usearch at the 476-node search space
    (N ~= 167,376 residues). v3's x0.4436 (213/476) down-scaling to the paper's
    213-seq space is DROPPED (decision 2), so e-values here are ~2.25x larger and
    the criterion is more conservative near the boundary. `--scale 1.0` == identity.
  * HSP selection: best HSP per unordered pair by MAX BITS (ties -> lower e-value),
    over both FASTA orientations (the adopted v3 s7 convention; accepted B4 cost).
  * Canonical component IDs resolve on the representative accession (G2: smallest
    accession per node), ordered by the smallest member accession per component.
...
def node_label(node):
    """Human-readable canonical label: accession, else enzyme name, else node id.
    v5 addition: 27 input rows carry an enzyme name but no accession, so an
    accession-only label would be blank for them (and for components they anchor)."""
    return node["accession"] or (node.get("enzyme_name") or "") or node["node_id"]


def acc_key(nodes, n):
    """Canonical ordering: real accessions first (alphabetical, as v4), blank-
    accession nodes last (ordered by enzyme name), node_id as final tiebreak.
    Accession-independent partition is unchanged; this only drives labels/IDs."""
    acc = nodes[n]["accession"]
    return (acc == "", node_label(nodes[n]), n)
...
def load_pairs(paths, scale=1.0):
    ...
    threshold. `scale` multiplies e-values to renormalize the search space; v4 uses
    1.0 (threshold the e-value as reported at the 476-node db)."""
    best = {}
    total = 0
    for path in paths:
...
def partition(prefix, scale=1.0):
    """Return (nodes, edge_list, comps, stats-fragment) for a node/pairs prefix."""
    ...
    for root, mem in groups.items():
        comps.append({"members": mem, "canon": min(mem, key=lambda n: acc_key(nodes, n))})
    # canonical order: by smallest member label (real accessions first)
    comps.sort(key=lambda c: acc_key(nodes, c["canon"]))
    for rank, c in enumerate(comps, start=1):
        c["component_id"] = f"C{rank:03d}"
        c["canonical_accession"] = node_label(nodes[c["canon"]])
...
def main():
    ...
    ap.add_argument("--date", default="2026-07-16")
    ap.add_argument("--tag", default="v4")
    ap.add_argument("--scale", type=float, default=1.0,
                    help="e-value renorm multiplier (v4=1.0: threshold at the run db)")
    args = ap.parse_args()
    ...
    key = lambda n: acc_key(nodes, n)

    comp_of = {n: c for c in comps for n in c["members"]}
    # size rank (1 = largest); ties broken by canonical accession order
    ...
    with (outdir / f"component_edges_{stem}.csv").open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["node_a", "node_b", "pct_id", "evalue", "bits", "acc_a", "acc_b",
                    "component_id"])
    ...
    # ---- per-node assignment (476 rows)
    with (outdir / f"component_assignment_{stem}.csv").open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["node_id", "sequence_md5", "accession", "accession_all",
                    "component_id", "size_rank", "sequence_length", "enzyme_name",
                    "organism", "v1_component", "cath"])
        for n in sorted(nodes, key=lambda n: (comp_of[n]["size_rank"], key(n))):
            r, c = nodes[n], comp_of[n]
            w.writerow([n, r["sequence_md5"], r["accession"], r["accession_all"],
    ...
    with (outdir / f"component_members_{stem}.csv").open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["component_id", "size_rank", "canonical_accession", "n_members",
        ...
        for c in sorted(comps, key=lambda c: c["size_rank"]):
            ...
            w.writerow([c["component_id"], c["size_rank"], c["canonical_accession"],
