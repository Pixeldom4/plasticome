...
set -euo pipefail
cd "$(dirname "$0")"                                   # -> <dataset>/usearch/
HERE_ABS="$(pwd -P)"
ROOT="$(cd ../.. && pwd -P)"                           # -> petadex-alignment/ (docker mount root)
REL="${HERE_ABS#"$ROOT"/}"                             # -> <dataset>/usearch
ANALYSIS=../../petadex-alignment-usearch/analysis
...
usearch() {  # $1 = fasta, $2 = out pairs  (both relative to this usearch/ dir)
  docker run --rm --platform linux/amd64 -v "$ROOT":/d -w /d debian:bookworm-slim \
    /d/petadex-alignment-usearch/analysis/bin/usearch11 -allpairs_local "/d/$REL/$1" \
    -userout "/d/$REL/$2" \
