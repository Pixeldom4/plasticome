...
#
# Steps 2 and 3 shell out to usearch, which is a Linux x86-64 ELF binary and runs
# only through Docker linux/amd64 on this arm64 host -- Docker Desktop must be up.
...
if want 2 || want 3; then
  ...
  [ -n "$DRY" ] || docker info >/dev/null 2>&1 \
    || die "Docker is not running — usearch is a Linux ELF binary and needs linux/amd64"
fi
...
if want 2 && ! skip 2 "$S2"; then
  banner 2 "reference-seeded clustering at ${ID} identity  [Docker]"
  run "$PY" "$REPO/lib/02 clustering/cluster_reference_seeded.py" \
      "$S1" "$SEEDS" --id "$ID" -o "$S2"
fi

if want 3 && ! skip 3 "$S3"; then
  banner 3 "all-vs-all alignment of the centroids → component_id  [Docker]"
  ...
  run env PY="$PY" V1="$V1" OUT_TSV="$S3" "$REPO/lib/03 alignment/run_from_clusters.sh" "$S2"
fi
