#!/usr/bin/env python3
"""Annotate the v8 cleaned PAZy final TSV in place with its v8 component.

Emits every source row in ORIGINAL order with the original columns preserved, plus
the v8 partition columns joined on sequence md5. md5-duplicate rows all receive the
...
The v8 input carries a pre-existing `component_id` column (prior assignment); it is
preserved as `prior_component_id` so the freshly computed v8 partition can be
compared against it. The de-novo partition never reads that prior column.
"""
import csv
import hashlib
import re
...
ASSIGN = ROOT / "component_assignment_v8_2026-07-20.csv"
OUT = ROOT / "cleaned_pazy_final_v8_components_2026-07-20.csv"
...
def main():
    ...
    with OUT.open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(SRC_COLS + ["prior_component_id", "node_status"] + add)
        for r in rows:
            ...
            if a:
                n_assigned += 1
                w.writerow([r[c] for c in SRC_COLS] + [prior, "assigned"] + [a[c] for c in add])
            else:
                n_blank += 1
                w.writerow([r[c] for c in SRC_COLS] + [prior, "no_sequence"] + [""] * len(add))
