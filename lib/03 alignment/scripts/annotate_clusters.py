#!/usr/bin/env python3
"""Join the computed partition back onto the cluster-centroids (or union) TSV.

Emits every input row in ORIGINAL order with all of its original columns
preserved, plus the partition columns joined on the md5 of the row's sequence
...
The sequence column is `rep_aa_sequence` for the clusters table and `aa_sequence`
for the union table (align before cluster); it is picked from the header, or
forced with `--seq-col`.
...
Rows whose sequences are byte-identical after normalize() collapse to one node
upstream and therefore land in the same component -- that is reported, not an
error. A row with no sequence gets empty partition columns and
...
def main() -> None:
    ...
    ap.add_argument("--clusters", type=Path, required=True,
                    help="the original clusters TSV (or union TSV)")
    ...
    ap.add_argument("--seq-col", default=None,
                    help="sequence column to join on (default: rep_aa_sequence, else aa_sequence)")
    args = ap.parse_args()
    ...
    seq_col = args.seq_col or next((c for c in ("rep_aa_sequence", "aa_sequence") if c in src_cols), None)
    if seq_col not in src_cols:
        sys.exit(f"[annotate_clusters] {args.clusters} has no "
                 f"{args.seq_col or 'rep_aa_sequence/aa_sequence'} column")
        ...
        with args.out.open("w", newline="") as fh:
            ...
            for r in rows:
                ...
                if a:
                    ...
                    n_missing += 1
                    status = "no_sequence" if not md5_of(r[seq_col]) else "unassigned"
    ...
    note = f", {shared} node(s) shared by >1 row (identical sequences)" if shared else ""
    if n_missing:
        note += f", {n_missing} row(s) not assigned"
    print(f"[annotate_clusters] wrote {args.out.name}: {len(rows)} rows on {seq_col} "
